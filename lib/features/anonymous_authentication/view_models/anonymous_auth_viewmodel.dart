...
import 'package:project_3_forex_signals_daily/features/anonymous_authentication/repositories/anonymous_auth_repository.dart';
import 'package:riverpod_annotation/riverpod_annotation.dart';
part 'anonymous_auth_viewmodel.g.dart';

@riverpod
class AnonymousAuthViewModel extends _$AnonymousAuthViewModel {
  // we put 'late' keyword because we're initializing _firebaseAuthRepository later, in build method
  late final AnonymousAuthRepository _anonymousAuthRepository;
  ...
  AsyncValue<User?> build() {
    ...
    _anonymousAuthRepository = ref.watch(anonymousAuthRepositoryProvider);
    ...
    final currentUser = _anonymousAuthRepository.getCurrentUser();
    if (currentUser != null) {
  ...
  Future<void> _checkAndSignIn() async {
    ...
    final signInResult = await _anonymousAuthRepository.signInAnonymously();
  ...
  void signOut() {
    state = const AsyncValue.loading();
    _anonymousAuthRepository.signOut();
    _checkAndSignIn();
  }

  User? getCurrentUser() {
    return _anonymousAuthRepository.getCurrentUser();
  }
}
